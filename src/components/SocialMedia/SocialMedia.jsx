...
const SocialMedia = () => {
  ...
  const { data, isLoading, refetch } = useGetSocialMediaQuery({
    page: page + 1,
    limit: rowsPerPage,
  });
  ...
  const [deleteSocialMedia, { isLoading: isDeleting }] =
    useDeleteSocialMediaMutation();
  const [updateStatus] = useUpdateSocialMediaStatusMutation();
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await deleteSocialMedia(itemToDelete.id).unwrap();
      setSnackbar({
        open: true,
            ...
            >
              ...
              >
                <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                  <Share2 size={22} color="var(--textColor)" />
                  <Typography
                    variant="h5"
                    sx={{
                      fontWeight: 600,
